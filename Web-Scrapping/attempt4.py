...
class PaloAltoScraper:
    ...
    def scrape(self):
        ...
        for idx, xp in enumerate(self.xpaths, start=1):
            ...
            for i, row in enumerate(rows):
                ...
                # For XPath 2, first column contains names, move to softwareName
                if idx == 2 and i == 0:
                    software_name = version  # move first column to softwareName
                    continue  # skip placeholder row

                # For special xpaths 9 & 11, first row version is the software name
                if idx in {9, 11} and special_first_version is None:
                    special_first_version = version
                    software_name = special_first_version
                    continue  # skip placeholder row

                # Shift version to name if it matches the special first row
                if idx in {9, 11} and version == special_first_version:
                    version = "-"
                    software_name = special_first_version
    ...
    def save_to_csv(self, filename="paloalto_software4.csv"):
